...
void CGameStateRun::OnInit()  								// 遊戲的初值及圖形設定
{
	...
	pratice.LoadBitmap("RES\\hi.bmp", RGB(255, 255, 255));
	gamemap.LoadBitmap();
...
CGameMap::CGameMap() :X(40), Y(60), MW(120), MH(100)
{
		...
		for (int j = 0; j < 4; j++)
		{
			switch (map[j][i])
			{
				case 0:
					break;
				case 1:
					blue.SetTopLeft(X + (MH*i), Y + (MH*j));
					blue.ShowBitmap();
					break;
				case 2:
					green.SetTopLeft(X + (MH*i), Y + (MH*j));
					green.ShowBitmap();
					break;
				default:
					ASSERT(0);
...
void CGameMap::InittializeBouncingBall(int ini_index, int row, int col)
{
	const int VELOCITY = 10;
	const int BALL_PICHEIGHT = 15;
	int floor = Y + (row + 1)*MH - BALL_PICHEIGHT;
...
void CGameMap::RandomBouncingBall()
{
	...
	for (int row = 0; row < 4; row++)
		for (int col = 0; col < 5; col++)
		{
			...
			{
				InittializeBouncingBall(ini_index, row, col);
				ini_index++;
			}
		}
}
